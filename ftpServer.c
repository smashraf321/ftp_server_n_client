...
{
	//signal(SIGINT, intHandler); //handle when user wants to end server
	...
	*/
	...
	while(keepRunning)
	{
		...
		printf(" now reading the client's request \n");
		...
		printf("\n Client's request of size %d was: %s \n",n_req,client_request);
		...
		if(strcmp(temp_request,client_request_list) == 0)
		{
			...
			write(acc, valid_request, sizeof(valid_request)); // sends OK to client
			...
			printf(" Client's wants to receive file list on port: '%d' \n",ntohs(client_list.sin_port));
