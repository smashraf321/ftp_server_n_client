...
int main(int argc, char* argv[])
{
 ...
 int i,j,k,sp_cnt; // support variables
 char ack[5]; // support variable
 //int/char ch; // support variable for alternate way of reading file
 ...
 */
 ...
 char temp_request[8] = " "; // for storing parsed request information from client like LIST, RET, etc.
 ...
 /*
 char client_request_list[5] = "LIST";
 char client_request_ret[4] = "RET";
 char valid_request[3] = "OK";
 char invalid_request[120] = "Right now the server supports only 2 commands, LIST and RETR, please try again";
 */
 //char uptime_msg[80] = " ";
 ...
 while(1)
 {
   ...
   acc = accept(control_sock, (struct sockaddr*) &client, (socklen_t *) &client_len);
   ...
   if((n_req = read(control_sock, client_request, 50)) < 0) // reads the client's request message
   {
	 perror("\n read() failed \n");
	 return -1;
   }
   ...
   if(strcmp(temp_request,client_request_list) == 0)
   {
     ...
     while(fgets(dir_list, sizeof(dir_list)-1, p) != NULL)
	 {
		 write(list_sock, dir_list, 20); // send the file list to client
		 read(list_sock, ack, 4); // wait for acknowledgment
	 }

	 pclose(p); // close the pipe from ls stream
   ...
   else if(strcmp(temp_request,client_request_ret) == 0)
   {
	 ...
	 for (i = 0, j = 0, k = 0; client_request[i] != '\r' ; i++)
	 {
		...
		if(sp_cnt == 1)
		{
			temp_request[j] = client_request[i];
			j++;
		}
	...
	*/
	 if( (fd = open(temp_request, O_RDONLY)) < 0)
	 {
	 ...
	 {
		 write(data_sock, data_buffer, nsent); // send the file list to client
